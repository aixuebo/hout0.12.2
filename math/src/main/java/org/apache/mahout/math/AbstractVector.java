...
public abstract class AbstractVector implements Vector, LengthCachingVector {
   ...
   * 
   * 每一个元素经过map方法处理,成新的值,新的值参与aggregator聚合,
   * 聚合函数两个参数,第一个累计值,第二个就是新元素用map处理后的值
   * 
   * 比如 聚合函数是a+b  map函数是平方
   * 数据分别是1 3 5 7 9
   * 则值是 1+9+25+49+81
   */
  @Override
  public double aggregate(DoubleDoubleFunction aggregator, DoubleFunction map) {
    ...
    if (aggregator.isAssociativeAndCommutative() && aggregator.isLikeLeftMult()
        && size > getNumNondefaultElements() && !map.isDensifying()) {//优化方式,如果函数符合优化方式的话,则会优化
      return 0;
    }

    double result;
    if (isSequentialAccess() || aggregator.isAssociativeAndCommutative()) {//优化方式,如果函数符合优化方式的话,则会优化
      ...
      result = map.apply(getQuick(0));//第一个函数经过map映射后的值
      for (int i = 1; i < size; i++) {//从第二个到最后一个函数,都和前面计算过的值进行交流
  ...
  public Vector divide(double x) {
    ...
    Vector result = createOptimizedCopy();//复制一份向量
  ...
  public double dot(Vector x) {
    ...
    if (this == x) {//如果向量相同,则直接返回已经计算好的值即可
  ...
  public double norm(double power) {
    ...
    if (Double.isInfinite(power)) {
      return aggregate(Functions.MAX, Functions.ABS);//返回整个向量中绝对值最大的元素
    } else if (power == 2.0) {
  ...
  public int maxValueIndex() {
    ...
    while (iter.hasNext()) {
      ...
      double tmp = element.get();//此时的值
      if (tmp > max) {
        max = tmp;//设置最大值
        result = element.index();//设置最大值对应的下标index
      }
    }
    ...
    //说明此时最大值还比0小,因此我们要找到第一个0的值就是最大值
  ...
  public int minValueIndex() {
    ...
    // unfilled element(0.0) could be the maxValue hence we need to
    // find one of those elements
    if (nonZeroElements < size && min > 0.0) {//最小值大于0,因此我们要找到第一个0的下标
      for (Element element : all()) {
        if (element.get() == 0.0) {
          return element.index();
        }
  ...
  public Vector assign(DoubleDoubleFunction f, double y) {
    ...
    invalidateCachedLength();//让平方长度无效
    return this;
  }

  //对每一个元素都进行DoubleFunction函数处理,返回新的元素
  @Override
  ...
  /**
   * 1.循环每一个元素,获取对应的value
   * 2.对于非0的value,打印格式如下
   * {dictionary[index]:value,dictionary[index]:value,}
   * @param dictionary 字典表,即字典的每一个位置对应向量每一个位置的含义
   * @return
   */
  public String toString(String[] dictionary) {
    StringBuilder result = new StringBuilder();
    result.append('{');
    for (int index = 0; index < size; index++) {//循环每一个元素
      double value = getQuick(index);//获取对应的value
      if (value != 0.0) {
