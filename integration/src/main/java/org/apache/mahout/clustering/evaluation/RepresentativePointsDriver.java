...
public final class RepresentativePointsDriver extends AbstractJob {
  ...
  public int run(String[] args) throws ClassNotFoundException, IOException, InterruptedException {
    addInputOption();
    addOutputOption();
    addOption("clusteredPoints", "cp", "The path to the clustered points", true);//集群点的路径
    addOption(DefaultOptionCreator.distanceMeasureOption().create());//度量方式
    addOption(DefaultOptionCreator.maxIterationsOption().create());//最大迭代次数
    addOption(DefaultOptionCreator.methodOption().create());//sequential or mapreduce
  ...
  public static void printRepresentativePoints(Path output, int numIterations) {
    for (int i = 0; i <= numIterations; i++) {//循环每一轮迭代
      Path out = new Path(output, "representativePoints-" + i);//找到每一轮迭代的输出
  ...
  public static void run(Configuration conf, Path clustersIn, Path clusteredPointsIn, Path output,
    ...
    writeInitialState(stateIn, clustersIn);//将input的中信内容切换到output/representativePoints-0下再次计算
    
    for (int iteration = 0; iteration < numIterations; iteration++) {//循环迭代次数
      log.info("Representative Points Iteration {}", iteration);
      // point the output to a new directory per iteration
      Path stateOut = new Path(output, "representativePoints-" + (iteration + 1));//新一轮的输出
      runIteration(conf, clusteredPointsIn, stateIn, stateOut, measure, runSequential);
      // now point the input to the old output directory
      stateIn = stateOut;//新一轮的输出变成输入
    }
  ...
  private static void writeInitialState(Path output, Path clustersIn) throws IOException {
    ...
    for (FileStatus dir : fs.globStatus(clustersIn)) {
      ...
      for (FileStatus part : fs.listStatus(inPath, PathFilters.logsCRCFilter())) {//过滤掉.crc结尾的 或者.或者_开头的文件
