...
public class CanopyClusterer {
   ...
   * 
   * @param point
   *            the point to be added 要处理的向量点
   * @param canopies
   *            the List<Canopy> to be appended 用于存储的中心向量点
   */
  public void addPointToCanopies(Vector point, Collection<Canopy> canopies) {
    boolean pointStronglyBound = false;//false表示要创建一个新的中心向量点
    for (Canopy canopy : canopies) {
    	//中心向量长度的平方     中心向量   我们的点向量
      double dist = measure.distance(canopy.getCenter().getLengthSquared(), canopy.getCenter(), point);//计算中心向量和该向量点的距离
      if (dist < t1) {
      ...
      pointStronglyBound = pointStronglyBound || dist < t2;//一旦pointStronglyBound是true了,就一定该点以后都是true,即不会被添加成中心点,该点是否为中心点,取决于是否小于t2
    }
    if (!pointStronglyBound) {//说明要创建一个新的中心向量点
      if (log.isDebugEnabled()) {
