 ...
 * 计算最近的n个user,Nearest通过UserSimilarity被定义
 * </p>
 */
public final class NearestNUserNeighborhood extends AbstractUserNeighborhood {
  
  private final int n;//n邻居大小,上限是模型中user的数量
  private final double minSimilarity;//作为邻居所要求的最小相似度
  ...
  /**
   * @param n neighborhood size; capped at the number of users in the data model  n邻居大小,上限是模型中user的数量
   * @param minSimilarity minimal similarity required for neighbors 作为邻居所要求的最小相似度
   * @param samplingRate percentage of users to consider when building neighborhood -- decrease to trade quality for
   *   performance 在构建邻居的时候,对用户的抽样比例,降低该值,可以提高性能
   * @throws IllegalArgumentException
  ...
  public NearestNUserNeighborhood(int n,
    ...
    int numUsers = dataModel.getNumUsers();//总user数量
    this.n = n > numUsers ? numUsers : n;//理论上n不可能比user数量还大
  ...
  public long[] getUserNeighborhood(long userID) throws TasteException {
    ...
    UserSimilarity userSimilarityImpl = getUserSimilarity();//返回计算user和user之间的相似度的对象
  ...
  private static final class Estimator implements TopItems.Estimator<Long> {
    private final UserSimilarity userSimilarityImpl;//如何计算user和user之间的相似度的对象
    private final long theUserID;//为该用户查找邻居
    private final double minSim;//最小相似度,大于该相似度的都可以作为邻居的候选人
