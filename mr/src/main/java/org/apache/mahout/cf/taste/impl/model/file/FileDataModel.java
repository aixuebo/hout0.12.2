 ...
 * 希望一个文件每一行存储格式为 userid  喜欢哪个物品  以及喜欢程度  操作时间,用逗号拆分
 * <p>
 ...
 * 除非该文件最近已经被加载了,否则当refresh(Collection)被调用的时候,该类要重新加载文件
 * </p>
 *
 * <p>
 ...
 * </p>
 * 这个类也寻找在文件相同目录下,有哪些更新的文件,要求寻找的文件名字要与数据文件名字前缀相同,
 * 这个机制允许应用去推送新的数据到新的文件中
 *
 * <p>
 ...
 * </p>
 * 一个小的不同点,要注意下,更新的文件也能够表达要删除哪个元素,使用123,456,格式,即以空的偏爱度+,结束,我们认为要删除user-item映射关系
 * <p>
 ...
 * the file!
 * </p>
 * 所有的item在文件中,必须表达成没有偏爱度,或者必须都得有偏爱度,不能混合方式,一些有偏爱度,一些没有偏爱度.
 * 并且总是相同的符号方式去拆分文件的每一行数据
 *
 * <p>
 ...
 * 这个类不能支持大数据量的使用(因为是基于内存操作的),如果数据量大,请使用JDBC和数据库技术
 * </p>
 *
 * <p>
...
public class FileDataModel extends AbstractDataModel {
  ...
  public static final long DEFAULT_MIN_RELOAD_INTERVAL_MS = 60 * 1000L; // 1 minute? 默认最小一分钟被加载一次,这个时间内再被调用加载,也是不会加载的
  private static final char COMMENT_CHAR = '#';//备注
  private static final char[] DELIMIETERS = {',', '\t'};//拆分一行内容的字符串

  private final File dataFile;//数据文件
  private long lastModified;//文件的最后修改时间
  private long lastUpdateFileModified;//获取同目录下最后一个文件更新的时间
  private final transient Splitter delimiterPattern;
  private final boolean hasPrefValues;//true表示该文件有偏爱值数据
  private DataModel delegate;
  private final ReentrantLock reloadLock;//在reload加载文件的时候,要锁住文件
  private final boolean transpose;//true表示允许将user和item顺序翻转过来
   ...
   * user own using regex pattern.如果文件不想使用默认的\t或者,拆分,则可以使用正则表达式自己定义
   * @throws IOException
   */
  public FileDataModel(File dataFile, boolean transpose, long minReloadIntervalMS, String delimiterRegex)
    ...
    this.lastUpdateFileModified = readLastUpdateFileModified();//获取同目录下最后一个文件更新的时间

    FileLineIterator iterator = new FileLineIterator(dataFile, false);//一行一行读取文件,false表示第一行不能忽略
    ...
    if (delimiterRegex == null) {
      delimiter = determineDelimiter(firstLine);//返回可能的拆分符号
      delimiterPattern = Splitter.on(delimiter);
    } else {
      delimiter = '\0';
      delimiterPattern = Splitter.onPattern(delimiterRegex);
      if (!delimiterPattern.split(firstLine).iterator().hasNext()) {//测试是否在第一行找到拆分符号,如果找不到,则抛异常
        throw new IllegalArgumentException("Did not find a delimiter(pattern) in first line");
      }
    }
    
    List<String> firstLineSplit = new ArrayList<>();//拆分后的数据集合
  ...
  protected void reload() {
    if (reloadLock.tryLock()) {//在reload加载文件的时候,要锁住文件
      try {
        delegate = buildModel();
  ...
  protected DataModel buildModel() throws IOException {
    ...
    //user---map<item,timestamps>
    FastByIDMap<FastByIDMap<Long>> timestamps = new FastByIDMap<>();

    if (hasPrefValues) {//有偏爱度

      if (loadFreshData) {//说明文件有更新

    	//user---Preference(user-item-preference偏爱度)
        FastByIDMap<Collection<Preference>> data = new FastByIDMap<>();
        FileLineIterator iterator = new FileLineIterator(dataFile, false);
        processFile(iterator, data, timestamps, false);

        for (File updateFile : findUpdateFilesAfter(newLastModified)) {//继续处理我在加载期间产生的文件
   ...
   * etc.
   * 在与数据文件相同的目录下,查看其他文件的更改时间,只要名字前缀与数据文件相同,并且更改时间比参数的大,我们就返回这些数据文件集合
   * 
   */
  private Iterable<File> findUpdateFilesAfter(long minimumLastModified) {
    ...
    Map<Long, File> modTimeToUpdateFile = new TreeMap<>();//key是文件的最后修改时间,value是对应的文件
    FileFilter onlyFiles = new FileFilter() {//仅仅读取文件
      @Override
      public boolean accept(File file) {
        return !file.isDirectory();
      }
    };
    for (File updateFile : parentDir.listFiles(onlyFiles)) {//循环数据文件同目录下所有文件
      String updateFileName = updateFile.getName();
      //找到同数据文件.前面相同的名字,并且不是数据文件本身,有过修改时间的文件
    ...
    return modTimeToUpdateFile.values();//返回修改的文件集合
  }
  ...
  /**
   * 
   * @param dataOrUpdateFileIterator 迭代一个文件的内容,一行一行迭代
   * @param data
   * @param timestamps
   * @param fromPriorData
   */
  protected void processFile(FileLineIterator dataOrUpdateFileIterator,
    ...
    while (dataOrUpdateFileIterator.hasNext()) {
      String line = dataOrUpdateFileIterator.next();//一行信息
      if (!line.isEmpty()) {
        processLine(line, data, timestamps, fromPriorData);
        if (++count % 1000000 == 0) {//每隔一定行数,打印内容
  ...
  protected void processLine(String line,
    ...
    if (line.isEmpty() || line.charAt(0) == COMMENT_CHAR) {//备注行则不处理
      return;
    }

    Iterator<String> tokens = delimiterPattern.split(line).iterator();//拆分
